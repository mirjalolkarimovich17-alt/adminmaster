  ...
  useEffect(() => {
    ...
    async function load() {
      const [{ data: s }, { data: b }] = await Promise.all([
        supabase.from('barbershops').select('name,location_link').eq('id', TENANT_ID).single(),
        supabase.from('barbers').select('id,name,profile_photo,rating').eq('tenant_id', TENANT_ID).eq('is_active', true),
      ])
      setShop(s)
      setBarbers(b ?? [])
      setLoading(false)
