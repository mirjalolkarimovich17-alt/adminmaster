...
export default function Booking() {
  ...
  // Telegram user pre-fill
  useEffect(() => {
    const tg = window.Telegram?.WebApp
    if (tg?.initDataUnsafe?.user) {
      const u = tg.initDataUnsafe.user
      setSel(s => ({ ...s, name: [u.first_name, u.last_name].filter(Boolean).join(' ') }))
    }
    tg?.expand?.()
  }, [])
  ...
  useEffect(() => {
    async function load() {
      const [{ data: sv }, { data: br }] = await Promise.all([
        ...
        supabase.from('barbers').select('id,name,profile_photo,rating,daily_start_time,daily_end_time').eq('tenant_id', TENANT_ID).eq('is_active', true),
      ])
      setServices(sv ?? [])
      setBarbers(br ?? [])
      setLoading(false)
  ...
  // Load slots when barber/date/service changes (step 2)
  useEffect(() => {
    if (step !== 2 || !sel.barber || !sel.date || !sel.service) return
    ...
    load()
  }, [step, sel.barber, sel.date, sel.service])

  async function confirm() {
    if (!sel.slot || !sel.service || !sel.barber) return
    setSubmitting(true)
    setError('')
